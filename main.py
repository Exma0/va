...
def get_data(url):
    try:
        # Timeout süresini biraz kısalttık ki worker takılı kalmasın
...
def main_controller():
    ...
    if channel_id:
        def generate():
            ...
            # Sürekli döngü
            while True:
                playlist, base = get_data(master_url)
                if not playlist:
                    time.sleep(1)
                    continue
                ...
                for line in lines:
                    ...
                    if line and not line.startswith("#"):
                        ...
                        if name not in played_segments:
                            try:
                                # stream=True ve daha büyük chunk_size kullanarak takılmayı önle
                                with requests.get(ts_url, headers=HEADERS, stream=True, timeout=10, verify=False) as r:
                                    if r.status_code == 200:
                                        # 32KB parçalar halinde hızlıca gönder
                ...
                # Playlist'in yenilenmesi için kısa bir bekleme
                time.sleep(1)
